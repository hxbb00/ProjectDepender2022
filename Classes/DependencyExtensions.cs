...
namespace ProjectDepender.Classes
{
    public static class DependencyExtensions
    {
        ...
        public static TimeSpan TestCycles(ObservableCollection<ReferenceItem> aReferenceItems, CyclesDependenciesViewModel aOut)
        {
            var serviceDependence = new Dictionary<string, List<string>>();
            foreach (var item in aReferenceItems)
            {
                if (!serviceDependence.ContainsKey(item.ParentName))
                {
                    serviceDependence.Add(item.ParentName, new List<string>());
                }

                serviceDependence[item.ParentName].Add(item.ChildName);
            }
            ...
            if(null != cycles)
            {
                ...
                {
                    aOut.CyclesItems.Add(new CyclesReferenceItem(item));
                }
            }
